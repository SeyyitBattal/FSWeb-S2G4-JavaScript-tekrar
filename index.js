...
function CemberinCevresi(yaricapG1) {
  const cevreCember = 2 * pi * yaricapG1;
  return cevreCember;
}
...
function CemberinAlani(yaricapG2, piSayisi) {
  const alanCember = piSayisi * Math.pow(yaricapG2, 2);
  return alanCember;
}
...
// 3b çözümü:
ucetambolunenler = [];
sayilar.forEach(function (uceBolunen) {
  if (uceBolunen % 3 === 0) ucetambolunenler.push(uceBolunen);
  return ucetambolunenler;
});
console.log("Üçe tam bölünen rakamlar: ", ucetambolunenler);

//3c çözümü:
ucebolunenlerintoplami = ucetambolunenler.reduce((total, herSayi) => {
  return total + herSayi;
}, 0);
console.log("Üçe tam bölünen sayıların toplamı: ", ucebolunenlerintoplami);

//3d çözümü
besyuzdenkucuksayilar = [];
besyuzdenkucuksayilar = sayilar.filter((kucukler) => kucukler < 500);
console.log("Beş yüzden küçük sayılar: ", besyuzdenkucuksayilar);

//3e çözümü
siralisayilar = [];
siralisayilar = besyuzdenkucuksayilar.sort((a, b) => a - b);
console.log("Beş yüzden küçük sayıların sıralaması: ", siralisayilar);
...
// console.log(`${sayı}, sayısı ${tekrarSayisi} kere tekrar edilmiştir`);
